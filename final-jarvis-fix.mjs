...
const files = [
  // 1. GLOBALS.CSS (Fixing Scrollbar & Animation)
  {
...
::-webkit-scrollbar-thumb:hover { background: #334155; }`
  ...
  // 2. LAYOUT.TSX (Strict Width Calculation to fix the Right Gap)
  {
    path: 'app/layout.tsx',
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // 3. DEV MODE OVERLAY (Fully Functional)
  {
    path: 'components/ui/DevModeOverlay.tsx',
    content: `"use client";
...
export const DevModeOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  // 4. GLOBE WIDGET (Pure CSS, Network Proof)
  {
...
export const GlobeWidget = () => {
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-between rounded-2xl bg-[#0B1120] border border-white/10 overflow-hidden shadow-2xl p-6 min-h-[350px]">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center w-full">
        {/* PURE CSS GLOBE (Never fails due to network) */}
        <div className="relative w-full max-w-[280px] aspect-square rounded-full border border-cyan-500/50 bg-[#02040A] overflow-hidden flex items-center justify-center shadow-[0_0_80px_rgba(34,211,238,0.4),inset_0_0_60px_rgba(34,211,238,0.2)]">
          <div className="absolute inset-0 rounded-full border border-cyan-500/30" style={{ transform: 'rotateX(70deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30" style={{ transform: 'rotateX(40deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30" style={{ transform: 'rotateX(-40deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30" style={{ transform: 'rotateX(-70deg)' }}></div>
          
          <div className="absolute inset-0 rounded-full border border-cyan-500/30 animate-[spin_10s_linear_infinite]" style={{ transform: 'rotateY(30deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30 animate-[spin_10s_linear_infinite]" style={{ transform: 'rotateY(60deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30 animate-[spin_10s_linear_infinite]" style={{ transform: 'rotateY(90deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30 animate-[spin_10s_linear_infinite]" style={{ transform: 'rotateY(120deg)' }}></div>
          <div className="absolute inset-0 rounded-full border border-cyan-500/30 animate-[spin_10s_linear_infinite]" style={{ transform: 'rotateY(150deg)' }}></div>
          
          <div className="w-1/2 h-1/2 bg-cyan-400/30 rounded-full blur-2xl animate-pulse"></div>
        </div>
  ...
  // 5. MAIN PAGE.TSX (Perfect Layout & Working Dev Mode)
  {
    path: 'app/page.tsx',
    content: `"use client";
...
export default function CommandDashboard() {
  ...
  return (
    <div className={\`w-full p-6 space-y-6 \${devMode ? 'border border-dashed border-accent/50' : ''}\`}>
        ...
        <div className="flex flex-col gap-6 h-full">
          ...
          <div className={\`flex-1 rounded-2xl bg-[#0B1120] border border-white/10 p-6 flex flex-col justify-between shadow-xl min-h-[300px] \${devMode ? 'border-dashed border-red-500/50' : ''}\`}>
            <div>
              ...
              {/* PURE CSS Heatmap (Prevents Network Errors) */}
              <div className="w-full h-40 rounded-xl relative overflow-hidden flex flex-col bg-[#010816] border border-cyan-900/50 shadow-[inset_0_0_20px_rgba(34,211,238,0.15)]">
                 <div className="absolute inset-0 bg-[linear-gradient(to_right,#22d3ee22_1px,transparent_1px),linear-gradient(to_bottom,#22d3ee22_1px,transparent_1px)] bg-[size:10px_10px]"></div>
        ...
        <div className="flex flex-col gap-6 h-full">
          ...
          <div className={\`flex-1 rounded-2xl bg-[#0B1120] border border-white/10 p-6 flex flex-col justify-between shadow-xl min-h-[300px] \${devMode ? 'border-dashed border-red-500/50' : ''}\`}>
            <div>
              ...
              <div className="w-full h-32 bg-gradient-to-br from-blue-900 to-[#0B1120] rounded-xl border border-white/10 mb-4 shadow-lg flex items-center justify-center overflow-hidden relative">
                 <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff10_1px,transparent_1px),linear-gradient(to_bottom,#ffffff10_1px,transparent_1px)] bg-[size:10px_10px]"></div>
                 <span className="text-white/30 font-mono text-xs z-10">Interactive FP&A Dashboard</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
        <div className={\`lg:col-span-2 rounded-2xl bg-[#0B1120] border border-white/10 p-6 overflow-x-auto custom-scrollbar shadow-xl \${devMode ? 'border-dashed border-red-500/50' : ''}\`}>
...
async function runFinalJarvisFix() {
  console.log('🚀 INITIALIZING JARVIS PROTOCOL FIX...');
  for (const file of files) {
    const fullPath = path.join(process.cwd(), file.path);
    await fs.mkdir(path.dirname(fullPath), { recursive: true });
    await fs.writeFile(fullPath, file.content, 'utf8');
    console.log(`✅ Deployed Network-Safe Component: ${file.path}`);
  }
  console.log('\\n🎉 SUCCESS! Layout centered. Dev Mode works. ADNOC is gone. Maps/Globe use Pure CSS.');
}
