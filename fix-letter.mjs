...
export default function AICFOPage() {
  ...
  const simulateStreamingResponse = (userText: string) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      // Cleaned up the raw markdown asterisks for a cleaner look
      ...
      const interval = setInterval(() => {
        // FIX: Extracting the exact character first before letting React process it!
        const charToAppend = aiResponse.charAt(i);
        ...
        setMessages(prev => {
          ...
          newMessages[lastIdx] = { 
            ...newMessages[lastIdx], 
            text: newMessages[lastIdx].text + charToAppend 
          };
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={\`flex gap-4 max-w-4xl \${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}\`}>
              ...
              <div className={\`p-5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap \${msg.role === 'user' ? 'bg-primary/10 border border-primary/20 text-white shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'bg-[#050816] border border-white/5 text-white/90 shadow-inner'}\`}>
                {msg.text}
                {msg.role === 'ai' && msg.text.length > 0 && idx !== 0 && !isTyping && (
                  <div className="mt-4 flex items-center gap-4 border-t border-white/10 pt-3 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-3 mb-5">
              {suggestedPrompts.map(prompt => (
                <button 
                  ...
                  className="text-[11px] font-mono text-accent bg-accent/5 border border-accent/20 px-4 py-2 rounded-full hover:bg-accent hover:text-black hover:shadow-[0_0_15px_rgba(34,211,238,0.4)] transition-all text-left"
                >
                  {prompt}
                </button>
              ))}
