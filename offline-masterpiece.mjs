...
const files = [
  // ==========================================
  // 1. GLOBE WIDGET (100% OFFLINE SCI-FI EARTH & HUD WEB)
  // ==========================================
  {
...
export const GlobeWidget = () => {
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center rounded-2xl bg-[#0B1120] border border-white/10 overflow-hidden shadow-2xl p-6 min-h-[360px]">
      ...
      <div className="relative z-10 flex items-center justify-center w-full">
        <div className="relative w-full max-w-[260px] aspect-square rounded-full shadow-[0_0_60px_rgba(34,211,238,0.2),inset_-20px_-20px_60px_rgba(0,0,0,0.9),inset_10px_10px_30px_rgba(34,211,238,0.2)] bg-gradient-to-tr from-[#021124] via-[#06263b] to-[#010814] overflow-hidden border border-cyan-500/40">
          ...
          {/* SVG Fractal Continents (Looks amazing, needs no internet) */}
          <div className="absolute top-0 left-0 w-[200%] h-full opacity-60 mix-blend-color-dodge" style={{ animation: 'spinProcedural 40s linear infinite' }}>
  ...
  // ==========================================
  // 2. PAGE.TSX (OFFLINE HEATMAP WITH CONNECTING LINES)
  // ==========================================
  {
    path: 'app/page.tsx',
    content: `"use client";
          ...
          <div className="flex-1 rounded-2xl bg-[#0B1120] border border-white/10 p-6 flex flex-col justify-between shadow-xl min-h-[300px]">
            <div>
              ...
              <div className="w-full h-40 rounded-xl relative overflow-hidden flex items-center justify-center bg-[#010816] border border-cyan-900/50 shadow-[inset_0_0_20px_rgba(34,211,238,0.15)]">
                ...
                {/* Procedural Data Cluster Map (No image needed!) */}
                ...
                <div className="absolute top-[28%] left-[27%] text-[8px] font-mono text-success z-30 drop-shadow-md bg-black/60 px-1 rounded backdrop-blur-sm border border-success/30">NYSE: +1.2%</div>
                ...
                <div className="absolute top-[38%] right-[27%] text-[8px] font-mono text-accent z-30 drop-shadow-md bg-black/60 px-1 rounded backdrop-blur-sm border border-accent/30">ASIAN MRKT: +3.4%</div>
                ...
                <div className="absolute bottom-[11%] left-[42%] text-[8px] font-mono text-warning z-30 drop-shadow-md bg-black/60 px-1 rounded backdrop-blur-sm border border-warning/30">GOLD: $2450</div>
                
                {/* Legend */}
                <div className="absolute bottom-3 left-3 flex flex-col gap-1 z-30 bg-black/50 p-1.5 rounded backdrop-blur-md border border-white/5">
...
async function installOfflineMasterpiece() {
  for (const file of files) {
    const fullPath = path.join(process.cwd(), file.path);
    await fs.mkdir(path.dirname(fullPath), { recursive: true });
    await fs.writeFile(fullPath, file.content, 'utf8');
    console.log(`✅ Offline Sci-Fi Elements Installed: ${file.path}`);
  }
}
installOfflineMasterpiece();