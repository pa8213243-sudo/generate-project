...
async function injectClickableJourney() {
  const filePath = path.join(process.cwd(), 'app/page.tsx');
  try {
    await fs.writeFile(filePath, pageContent, 'utf8');
    console.log('✅ Success! The "My Journey" card is now clickable and routes directly to Career Trajectory.');
  } catch (error) {
    console.log('❌ Error:', error);
  }
}

injectClickableJourney();