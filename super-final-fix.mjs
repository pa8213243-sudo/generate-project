...
const files = [
  // ==========================================
  // 1. GLOBE WIDGET (SOLID, GLOWING EARTH - NO TRANSPARENT LINES)
  // ==========================================
  {
...
export const GlobeWidget = () => {
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-between rounded-2xl bg-[#0B1120] border border-white/10 overflow-hidden shadow-2xl p-6 min-h-[350px]">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center w-full">
        {/* SOLID ROTATING EARTH */}
        <div className="relative w-full max-w-[280px] aspect-square rounded-full shadow-[0_0_60px_rgba(34,211,238,0.4),inset_-20px_-20px_50px_rgba(0,0,0,0.9)] bg-black overflow-hidden border border-cyan-500/30">
          <div 
             className="absolute inset-0 w-full h-full opacity-100 animate-earth"
             style={{
               backgroundImage: "url('https://upload.wikimedia.org/wikipedia/commons/c/c4/Earthmap1000x500compac.jpg')",
               backgroundSize: "200% 100%",
               filter: "brightness(1.2) contrast(1.2) hue-rotate(15deg) saturate(1.5)"
             }}
          />
        </div>
        
        {/* Front Glass/Glow Overlay over the Earth */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[280px] aspect-square rounded-full bg-gradient-to-br from-cyan-300/20 to-transparent pointer-events-none mix-blend-overlay" />
      </div>
  ...
  // ==========================================
  // 2. PAGE.TSX (DOUBLE MARGIN BUG FIXED, EXACT PROPORTIONS)
  // ==========================================
  {
    path: 'app/page.tsx',
    content: `"use client";
...
export default function CommandDashboard() {
  ...
  return (
    // FIX: Removed 'lg:ml-64' because layout.tsx already has it. This fixes the right-side leaning/gap!
    <div className={\`w-full p-6 space-y-6 max-w-[1600px] mx-auto \${devMode ? 'border border-dashed border-accent/50' : ''}\`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full items-stretch">
        ...
        <div className="flex flex-col gap-6 h-full">
          ...
          {/* Real World Heatmap Image restored */}
          <div className="flex-1 rounded-2xl bg-[#0B1120] border border-white/10 p-6 flex flex-col justify-between shadow-xl min-h-[300px]">
            <div>
              ...
              <div className="w-full h-40 rounded-xl relative overflow-hidden flex items-center justify-center bg-[#010816] border border-cyan-900/50 shadow-[inset_0_0_20px_rgba(34,211,238,0.15)]">
                <div className="absolute inset-0 bg-[url('https://upload.wikimedia.org/wikipedia/commons/thumb/e/ec/World_map_blank_without_borders.svg/1000px-World_map_blank_without_borders.svg.png')] bg-contain bg-center bg-no-repeat opacity-80 filter invert sepia hue-rotate-[130deg] saturate-[3] brightness-110" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
        <div className="lg:col-span-2 rounded-2xl bg-[#0B1120] border border-white/10 p-6 overflow-x-auto custom-scrollbar shadow-xl">
...
// 3. SCRIPT TO FIX THE UBER IMAGES IN DATASERVICE.TS
async function fixUberImages() {
  const dataPath = path.join(process.cwd(), 'services/dataService.ts');
  try {
    let content = await fs.readFile(dataPath, 'utf8');
    
    // Replace incorrectly repeating Uber images with their specific images
    content = content.replace(/(title:\s*['"]Croma Sales Dashboard['"][\s\S]*?image:\s*['"])[^'"]+(['"])/g, '$1/projects/sales dashboard.jpeg$2');
    content = content.replace(/(title:\s*['"]Flipkart Sales Dashboard['"][\s\S]*?image:\s*['"])[^'"]+(['"])/g, '$1/projects/flipcart.jpeg$2');
    content = content.replace(/(title:\s*['"]Android App['"][\s\S]*?image:\s*['"])[^'"]+(['"])/g, '$1/projects/sales dashboard.jpeg$2'); 
    
    await fs.writeFile(dataPath, content, 'utf8');
    console.log('✅ Fixed Uber Image duplication in dataService.ts');
  } catch (err) {
    console.log('⚠️ Could not process dataService.ts. Please verify path if needed.', err.message);
  }
}

async function runSuperFinalFix() {
  console.log('🚀 EXECUTING ABSOLUTE FINAL FIX...');
  ...
  console.log('\\n🎉 DONE. Layout centered. Globe is bright and solid. Uber images fixed.');
}
